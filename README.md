...
Creating an app requires an S3-compatible object-storage integration that supplies the GitLab connection YAML and
distinct, pre-created bucket names for artifacts, LFS objects, uploads, packages, backups, and temporary restore data.
...
The backup bucket names reserve the object-storage locations expected by GitLab, but this stack does not yet automate
a complete GitLab backup or restore. Database backups or volume snapshots alone are insufficient: a recoverable GitLab
backup must cover the database, repositories, object-storage data, and a separately protected copy of the Rails
secrets. Follow the [GitLab backup documentation](https://docs.gitlab.com/charts/backup-restore/) before using the stack
for production data.
